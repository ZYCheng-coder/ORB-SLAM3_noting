...
namespace ORB_SLAM3 {
    ...
    Settings::Settings(const std::string &configFile, const int &sensor) :
        ...
        if (!fSettings.isOpened()) {
            LOG(ERROR) << "[ERROR]: could not open configuration file at: " << configFile << endl;
            LOG(ERROR) << "Aborting..." << endl;

            exit(-1);
        } else {
            LOG(INFO) << "Loading settings from " << configFile << endl;
        }

        // 获取相机内参等
        readCamera1(fSettings);
        LOG(INFO) << "\t-Loaded camera 1" << endl;

        //Read second camera if stereo (not rectified)
        // 如果双目相机，获取另外相机的内参
        if (sensor_ == System::STEREO || sensor_ == System::IMU_STEREO) {
            readCamera2(fSettings);
            LOG(INFO) << "\t-Loaded camera 2" << endl;
        }

        // 获取图片参数，如长宽 | new 长宽
        readImageInfo(fSettings);
        LOG(INFO) << "\t-Loaded image info" << endl;

        if (sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO || sensor_ == System::IMU_RGBD) {
            readIMU(fSettings);
            LOG(INFO) << "\t-Loaded IMU calibration" << endl;
        }

        if (sensor_ == System::RGBD || sensor_ == System::IMU_RGBD) {
            readRGBD(fSettings);
            LOG(INFO) << "\t-Loaded RGB-D calibration" << endl;
        }

        readORB(fSettings);  // 读取ORB参数
        LOG(INFO) << "\t-Loaded ORB settings" << endl;
        readViewer(fSettings);  //读取可视化参数
        LOG(INFO) << "\t-Loaded viewer settings" << endl;
        readLoadAndSave(fSettings);
        LOG(INFO) << "\t-Loaded Atlas settings" << endl;
        readOtherParameters(fSettings);
        LOG(INFO) << "\t-Loaded misc parameters" << endl;

        if (bNeedToRectify_) {
            precomputeRectificationMaps();
            LOG(INFO) << "\t-Computed rectification maps" << endl;
        }
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        if (cameraModel == "PinHole") {
            ...
            LOG(ERROR) << "Error: " << cameraModel << " not known" << endl;
            exit(-1);
        }
    }
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if (found) {
            ...
            if (!bNeedToRectify_) {  // 加载相机2时，设置为true
                //Update calibration
